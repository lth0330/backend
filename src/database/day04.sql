...
# [1] 테이블에서 그룹절 : group by 절;
# select 속석명 from 테이블명 group by 그룹기준
# 주의할점 : 그룹 기준외 다른 속성들에 대해 어떻게 보여줄지 정의가 안됨.
...
select max(bamount) from buy; -- buy 테이블내 bamount 속성값들의 전체 중 최대값
...
-- select mid, sum(bamout) from buy group by mid;   이건 왜 안되지
