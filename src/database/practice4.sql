...
-- [문제 3] orders 테이블에서 도서번호(book_id)별 주문이 존재하는 도서 목록을 조회하세요. (중복 제거)
	select distinct book_id from orders where book_id is not null;
...
-- 출력: customer, 총구매금액
	 ...
	 -- select customer, sum(order_qty * price) from orders, books group by 아직 아닌ㅁ
...
-- 출력: customer, 총주문수량
-- 조건: HAVING 사용
	select customer,sum(order_qty) from orders group by customer having count(order_qty) >=5;
